...
export interface SupabaseHarness {
  ...
  storageRemoveError: PostgrestError | null
  calls(): HarnessCall[]
  callsOf(table: string, method?: string): HarnessCall[]
  rpcCalls(): HarnessRpcCall[]
  rpcOf(fn: string): HarnessRpcCall[]
  when(table: string, method: 'insert' | 'update' | 'select' | 'delete', data: unknown): void
  whenCount(table: string, count: number): void
  fail(table: string, method: 'insert' | 'update' | 'select' | 'delete', error: PostgrestError): void
...
function outcome(harness: Core, table: string, chain: Chain): HarnessResult {
  ...
  const bucket = chain.calls.some((c) => c.method === 'delete')
    ? 'delete'
    : chain.calls.some((c) => c.method === 'update')
      ? 'update'
...
type Core = {
  ...
  storageRemovals: SupabaseHarness['storageRemovals']
  storageRemoveError: SupabaseHarness['storageRemoveError']
}
...
export function createSupabaseHarness(): SupabaseHarness {
  const core = {
    ...
    storageRemovals: [] as SupabaseHarness['storageRemovals'],
    storageRemoveError: null as SupabaseHarness['storageRemoveError'],
  } satisfies Core
  ...
  return {
    ...
    storageRemovals: core.storageRemovals,
    get storageRemoveError(): SupabaseHarness['storageRemoveError'] {
      return core.storageRemoveError
    },
    set storageRemoveError(value: SupabaseHarness['storageRemoveError']) {
      core.storageRemoveError = value
    },
      ...
      storage: {
        from: (bucket: string) => ({
          remove: async (paths: string[]) => {
            core.storageRemovals.push({ bucket, paths })
            if (core.storageRemoveError) {
              return { data: null, error: core.storageRemoveError }
            }
    ...
    when(table: string, method: 'insert' | 'update' | 'select' | 'delete', data: unknown) {
      core.results[table] ??= {}
    ...
    fail(table: string, method: 'insert' | 'update' | 'select' | 'delete', error: PostgrestError) {
      core.errors[table] ??= {}
    ...
    reset() {
      ...
      // مسح سجل التخزين أيضاً — لكن الإبقاء على نفس المرجع حتى لا يكسر
      // أي اختبار سابق التقط المصفوفة مباشرة.
      core.storageRemovals.length = 0
      core.storageRemoveError = null
    },
  }
}