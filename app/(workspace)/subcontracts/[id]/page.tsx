  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-bold text-ink sm:text-2xl">{order.contractor_name}</h1>
            ...
            <p className="mt-1 text-sm text-secondary">
              ...
              {project?.status && project.status !== 'active' && (
                <span className="ms-2 text-xs text-secondary">({project.status})</span>
              )}
            </p>
            <p className="mt-3 text-sm text-ink">{order.description}</p>
          </div>
        </header>
        ...
        <section className="mb-6 overflow-hidden rounded-2xl border border-secondary/30 bg-white shadow-lg shadow-primary/5">
          <div className="divide-y divide-secondary/20">
            <div className="flex items-center justify-between px-6 py-4">
              <span className="text-ink">المبلغ المتفق عليه</span>
              <span className="font-semibold tabular-nums text-primary">{formatCurrency(totalAgreed)}</span>
            </div>
            <div className="flex items-center justify-between px-6 py-4">
              <span className="text-ink">إجمالي المدفوع</span>
              <span className="font-semibold tabular-nums text-primary">{formatCurrency(paid)}</span>
            </div>
            <div className="flex items-center justify-between px-6 py-4">
              <span className="text-ink">عدد الدفعات (تشمل الملغاة)</span>
              <span className="font-semibold tabular-nums text-primary">{payments?.length ?? 0}</span>
            </div>
            <div className="flex items-center justify-between px-6 py-4">
              <span className="text-ink">تاريخ الإنشاء</span>
