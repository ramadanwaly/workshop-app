...
# Rules 1+2 allowlist: the single deliberate anon grant — increment_rate_limit
# must be callable by unauthenticated login posts (P2-04, migration 25).
# The function only writes rate counters for well-formed buckets.
ALLOW_ANON_GRANT='GRANT EXECUTE ON FUNCTION public\.increment_rate_limit\(TEXT, INT, INT\) TO anon'
if grep -rhn --include='*.sql' -E '^[[:space:]]*GRANT[[:space:]]' "$MIGR" | strip_comments | grep -E 'TO[[:space:]]+(PUBLIC|anon)([^_a-zA-Z]|$)' | grep -v -E "$ALLOW_ANON_GRANT" ; then
...
# Allowlist: the single deliberate anon grant — increment_rate_limit must be
# callable by unauthenticated login posts (P2-04, migration 25). The function
# only writes rate counters for well-formed buckets.
if grep -rhn --include='*.sql' -E 'TO[[:space:]]+(PUBLIC|anon)([^_a-zA-Z]|$)' "$MIGR" | strip_comments | grep -v -E '^[0-9]+:[[:space:]]*REVOKE' | grep -v -E "$ALLOW_ANON_GRANT" ; then
